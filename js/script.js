const global = {
  ...
  api: {
    apiKey: "7c25cd891d0d268c329b789e29ced872",
...
function displayPagination() {
  ...
  //Disabled Prev button if on first page
...
async function displayPopularShows() {
  const { results: showsResults } = await fetchAPIData("tv/popular");
  showsResults.forEach((show) => {
    const div = document.createElement("div");
    div.classList.add("card");
    div.innerHTML = `
...
async function displayPopularMovies() {
  const { results: movieResults } = await fetchAPIData("movie/popular");
      ...
      />`
        ...
        <div class="details-bottom">
          ...
          <ul>
            ...
            <li><span class="text-secondary">No. of Seasons:</span> ${
...
async function fetchAPIData(endpoint) {
  ...
  const response = await fetch(
    ...
    //https://api.themoviedb.org/3/movie/popular?api_key=12345&language=en-US
  );
